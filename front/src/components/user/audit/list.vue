  ...
  <div class='user-booking-list'>
    <table-page :data="localValue">
      ...
      <el-table-column label="预约结束时间">
        <template slot-scope="scope">
           {{ humanizeTime(scope.row.end_time) }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态">
        <template slot-scope="scope">
          <el-tag :type="bookingStatusStyle[scope.row.status]">
            {{ bookingStatusMapForAudit[scope.row.status] }}
          </el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    onApprove(row) {
      const textTips = `此操作将同意 ${row.booking_name} 的预约申请操作，请输入回复信息:`
    ...
    onReject(row) {
      const textTips = `此操作将拒绝 ${row.booking_name} 的预约申请操作，请输入拒绝原因:`
